...
import { NavbarProps } from '../constats/types';
        ...
        <nav className="flex justify-between items-center mx-auto mt-2 p-6 rounded-full" style={{ width: '80vw', height: '5rem', position: 'fixed', top: 0, zIndex: 2, left: '10%', backgroundColor: 'rgba(0, 0, 0, 0.3)', boxShadow: ' 0 4px 6px rgba(0, 0, 0, 0.6)', backdropFilter: 'blur(15px)' }}>
          {NAV_TABS.map((tab, index) => (
            <button
              ...
              className={`p-1 sm:p-2 text-xs rounded-full bg-transparent hover:border-emerald-400 focus:outline-none truncate ${activeIndex === index ? 'active' : ''}`}
              style={{ transition: 'background-color 0.3s ease', maxWidth: (100 / 5) - 3 + '%', overflow: 'hidden' }}
            >
              {tab.title}
            </button>
          ))}
