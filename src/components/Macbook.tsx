...
const Macbook = () => {
    ...
    const screenPosition = [0.37, 0.21, 0];
    const screenScale = [0.6, 0.6, 0.6];
    ...
    useEffect(() => {
        if (macbookRef.current) {
          ...
          gsap.fromTo(
            ...
            { y: 2.4, z: 0,  duration: 1.5, ease: "power2.out" } // Target rotation
          );
        }
        return () => {
    ...
    return (
      <group ref={macbookRef}>
        <primitive object={scene} ref={macbookRef} position={[0, -0.2, 0]} scale={[3, 3, 3]}  />;
...
const MacbookScene = () => {
  return (
    <Canvas camera={{ position: [1, 1, 1], fov: 80 }}>
